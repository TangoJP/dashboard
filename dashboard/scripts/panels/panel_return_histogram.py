...
with open('dashboard/scripts/json/widgets.json') as json_widgets:
    ...
    widget_settings = widget_settings['panel_return_histogram']
...
def panel_return_histogram(data):

    def get_data():
        period = selector_period.value
        data['return'] = calculate_percent_return(data['close'], period)

        metric = selectort_metric.value
        quantile = selector_quantile.labels[selector_quantile.active]
        name_column_quantile = metric + '_' + quantile
        ...
        # Remove any nan (of float type)
        unique_quantiles = [q for q in unique_quantiles if type(q) == str]
    ...
    def draw(source):
        ...
        p.quad(
            ...
            fill_alpha = 0.5, hover_fill_alpha=1.0, line_color='black'
        )
    ...
    selectort_metric.on_change('value', lambda attr, old, new: update())
